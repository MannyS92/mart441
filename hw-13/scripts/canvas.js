...
function setup()
{
    ...
    // create two objects
    square1 = new Square(200,200,50,50,"#cc0000");
    square2 = new Square(300,400,200,200,"#A9A9A9");
    $.getJSON("data/information.json", function(data) {
      ...
      $.getJSON("data/information2.json", function(data) {
        for(var i = 0; i < data.pSquares.length; i++)
        {
          squareArray.push(new Square(data.pSquares[i].x,data.pSquares[i].y, data.pSquares[i].h, data.pSquares[i].w, data.pSquares[i].color));
        }
        drawSquare();
          });
